...
public class ClimberSetRightPower extends CommandBase 
{
  // 1 for extned, -1 for retract
  private int m_type = 1;

  public ClimberSetRightPower(int type)
  {
  ...
  public void execute()
  {
    ...
    {
      RobotContainer.m_climber.setRightReelPower(m_type * Constants.CLIMBER_REEL_EXTEND_POWER);
      RobotContainer.m_climber.setRightLiftPower(-m_type * Constants.CLIMBER_LIFT_UNREEL_POWER);
    }
    else if (RobotContainer.m_OI.getOperatorController().getRawButton(Constants.OPERATOR_CLIMBER_LIFT_BTN_ID))
    {
      RobotContainer.m_climber.setRightLiftPower(m_type * Constants.CLIMBER_LIFT_EXTEND_POWER);
    }
    else
    {
